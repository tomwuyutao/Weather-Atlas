...
extension CityWeather {
  ...
  func displayedCondition(
    ...
    cityCalendar.timeZone = timeZone
    if cityCalendar.isDate(forecast.date, inSameDayAs: .now) {
  ...
  func recommendation(
    ...
    guard
      let forecast = forecastIfAvailable(
        on: date,
    ...
    return PlaceRecommendation(
      ...
      sunnyHourCount: Double(
        sunnyHoursData.hours.count(where: {
...
final class WeatherModel {
  ...
  var currentLocationPlaceCity: City? {
    ...
    let resolvedName =
      locality.flatMap { $0.isEmpty ? nil : $0 }
      ?? baseCity.name
    let resolvedCountry =
      country.flatMap { $0.isEmpty ? nil : $0 }
      ?? baseCity.country
  ...
  var currentLocationWeatherPlaceID: City.ID? {
    if let homeLocation {
  ...
  @ObservationIgnored
  private var placeNearbySnapshotsByOriginID: [City.ID: PlaceNearbySearchSnapshot] = [:]
  ...
  @ObservationIgnored var activeMapCandidateIDs: Set<City.ID> = []
  ...
  @ObservationIgnored private(set) var mapCandidateScopeGeneration = 0
    ...
    self.init(
      ...
      initialHomeLocation: {
        guard
          ...
          PlacesLibraryValidator.isValidCity(city)
        else {
          return nil
        }
        return city
  ...
  func ensureCurrentLocationWeather(
    ...
    guard !Task.isCancelled,
      ...
      makeLocationCity(coordinate: latestCoordinate).id == currentCity.id
    else {
      return
    }
    ...
    if let weather {
      ...
      if !isUsingHomeLocation {
        UserDefaults.standard.set(
          ...
          forKey: "weatherAtlas.currentLocationWeatherPlaceID"
        )
      }
    } else {
      locationError = localizedString(
  ...
  func searchNearbyPlaces(
    ...
    let hasCurrentWeather = {
      ...
      return locationWeather?.id == currentLocationWeatherPlaceID
    }()
    ...
    do {
      ...
      #endif
      guard !Task.isCancelled, refreshID == generation else { return }
  ...
  func searchNearbyPlaces(
    ...
    if !forceRefresh, let fallbackSnapshot {
      ...
      placeNearbyCandidates = fallbackSnapshot.candidates
      retainedPlaceNearbyIDs = fallbackSnapshot.retainedPlaceIDs
    ...
    do {
      ...
      placeNearbySnapshotsByOriginID[origin.id] = PlaceNearbySearchSnapshot(
        searchKey: key,
      ...
      placeNearbySnapshotRecency.removeAll { $0 == origin.id }
      placeNearbySnapshotRecency.append(origin.id)
  ...
  func rankedNearbyRecommendations(
    ...
    let originSunnyHourCount =
      originWeather.flatMap {
    ...
    for candidate in nearbyCandidates {
      ...
      guard
        ...
        recommendation.sunnyHourCount > originSunnyHourCount
      else {
        continue
      }
      candidates.append(
  ...
  func recordRecentCityAccess(_ city: City) {
    let retainedRecentCities = recentSearches.cities.filter { city in
    ...
    if retainedRecentCities != recentSearches.cities {
      recentSearches.cities = retainedRecentCities
    ...
    guard PlacesLibraryValidator.isValidCity(city),
      recentSearches.cities.first != city
    else {
      return
    }
    recentSearches.cities.removeAll {
      CitySemanticMatcher.matches($0, city)
    }
    recentSearches.cities.insert(city, at: 0)
    recentSearches.cities = Array(
      recentSearches.cities.prefix(
        RecentSearchStore.maximumSuggestionCount * 4
      )
    )
    recentSearches.persist()
  }
  ...
  func resetLocation() {
    ...
    homeLocation = nil
    UserDefaults.standard.removeObject(forKey: "weatherAtlas.homeLocation")
  ...
  func clearLocationState(keepingTransientCities: Bool) {
  ...
  private var physicalLocationFallbackPlaceID: City.ID? {
    ...
    guard let catalog = WidgetDataStore.catalog(),
      ...
      let longitude = previousLocation.longitude
    else {
      return nil
    }
  ...
  private func widgetLegacyIdentifiers(
    ...
    let canonicalIdentifierByPlaceID = Dictionary(
      uniqueKeysWithValues: places.map {
        ($0.id, "saved-place:" + $0.id.uuidString.lowercased())
      }
    )
    ...
    var priorCanonicalClaims:
      [String: [(placeID: SavedPlace.ID, isOwnMetadata: Bool, order: Int)]] = [:]
    for (order, previousCity) in previousCities.enumerated() {
      guard
        let previousPlaceID = WidgetDataStore.savedPlaceID(
    ...
    func ownerForUnmigratedCity(
      ...
      for (order, place) in places.enumerated() {
        let exactIdentifier =
          currentLegacyIdentifierByPlaceID[place.id]
          == previousCity.id
    ...
    where WidgetDataStore.savedPlaceID(from: previousCity.id) == nil {
      guard previousCity.id != WidgetDataStore.currentLocationIdentifier,
        let owner = ownerForUnmigratedCity(previousCity)
      else {
        continue
      }
      for alias
  ...
  private func widgetCurrentLocation(locale: Locale) -> WidgetDataCity? {
    ...
    let metadata = locationProvider.metadata
    let canonicalDisplayName =
      (currentLocationPlaceCity?
      .localizedDisplayName(locale: locale) ?? "")
      .trimmingCharacters(in: .whitespacesAndNewlines)
    let displayName =
      canonicalDisplayName.isEmpty
      ? localizedString("Current Location", locale: locale)
      : canonicalDisplayName
    let timeZoneID =
      locationWeather?.timeZone.identifier
      ?? currentLocationPlaceCity?.timeZoneIdentifier
