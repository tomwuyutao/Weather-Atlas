...
struct MapSunSearchResult: Identifiable {
  ...
  var id: City.ID { recommendation.cityWeather.city.id }
}
...
struct MapView: View {
  ...
  @State var presentedError: MapUIError?
  ...
  @State var acknowledgedSavedPlaceIDsByResultID: [City.ID: SavedPlace.ID] = [:]
  ...
  private var sortedPresentations: [PlacesMapPlacePresentation] {
    let orderedRecommendations = PlaceRecommendation.ranked(
      ((placesStore.allPlaces).map { place in
    ...
    let presentationsByID = Dictionary(
      uniqueKeysWithValues: ((placesStore.allPlaces).map { place in
        let weather = weatherStore.weather(for: place.id)
        return PlacesMapPlacePresentation(
          place: place,
          recommendation: weather.flatMap {
    ...
    let unavailable =
      ...
      .sorted {
        $0.place.localizedDisplayName(locale: locale).localizedStandardCompare(
          $1.place.localizedDisplayName(locale: locale)
        ) == .orderedAscending
      }
  ...
  private var mapCities: [City] {
    var cities = (placesStore.allPlaces).map(\.city)
  ...
  private var savedPlaceIDsByTransientResultID: [City.ID: SavedPlace.ID] {
    ...
    if let previewResult =
      ...
      !results.contains(where: {
        $0.id == previewResult.id
      })
    {
      results.append(previewResult)
    }
    for result in results {
      if let savedID = placesStore.savedPlaceID(
        matching: result.recommendation.cityWeather.city
      ) {
  ...
  private var locationRecommendation: PlaceRecommendation? {
    let hasCurrentWeather = {
      guard let coordinate = model.locationProvider.coordinate,
        CLLocationCoordinate2DIsValid(coordinate)
      else {
        return false
      }
      return model.locationWeather?.id == model.currentLocationWeatherPlaceID
    }()
    guard hasCurrentWeather,
      let weather = model.locationWeather
    else {
      return nil
    }
  ...
  private func updateVisibleMapWeatherAlert() async {
    ...
    await missingDataAlerts.retryThenReport(
      ...
      retry: {
        await weatherStore.load(cities: cities, forceRefresh: true)
      },
      isStillMissing: {
        ({ () -> Bool in
  ...
  var body: some View {
    mapBody
      .navigationTitle(localizedString("Map", locale: locale))
      ...
      .task(
        id: {
          ...
          let allPlaceIDs =
            ...
            .sorted()
            .joined(separator: ",")
          let previewID = router.mapPreviewCity?.id.uuidString ?? ""
          return [
            allPlaceIDs,
  ...
  private var mapToolbar: some ToolbarContent {
    ...
    ToolbarItem(placement: .topBarTrailing) {
      TopForecastDateSwitcher(
        ...
        availableDates: activeSunQuery != nil
          || isFindingSun
          || !sunCandidateCities.isEmpty
          ? Array(
            Set(
              sunSearchCandidateForecastDates
                + [model.forecastCalendar.startOfDay(for: selectedDate)]
            )
          ).sorted()
          : ordinaryMapForecastDates
      )
    }
  }
  ...
  private var ordinaryMapForecastDates: [Date] {
    ...
    let fallbackDates = (0..<10).compactMap {
      calendar.date(
        byAdding: .day,
        value: $0,
    ...
    let actualDates = Set(
      weatherSnapshots.flatMap { weather in
  ...
  private var mapBody: some View {
    PlacesMapCanvas(
      ...
      latestCachedWeatherDate: {
        _ = weatherStore.weatherRevision
        return weatherStore.refreshDatesByPlaceID.values.max()
      }(),
      ...
      locationName: model.currentLocationPlaceCity?
      ...
      isLocationWeatherLoading: (model.isRefreshingLocation
      ...
      isPreviewWeatherLoading: ({ () -> Bool in
      ...
      displayName: { $0.localizedDisplayName(locale: locale) },
      findSunHere: {
        beginSunSearch(.area)
      },
      findSunNearMe: {
        ...
        {
          beginSunSearch(.nearPlace(homeCity))
        } else {
      ...
      mapPlaceCardWeatherRevision: ("\(weatherStore.weatherRevision)-\(selectedDate.timeIntervalSinceReferenceDate)"),
  ...
  private func consumePendingSunHandoff(requestID: Int) {
    guard requestID > 0,
      let handoff = router.pendingMapSunHandoff
    else {
      return
    }
  ...
  func prepareForIncomingMapHandoff(token: Int) {
    ...
    sunSearchResults = []
    sunCandidateCities = []
    if !model.activeMapCandidateIDs.isEmpty {
      model.activeMapCandidateIDs = []
...
private struct MapFloatingLocationButton: View {
  ...
  var body: some View {
    if reduceTransparency {
      ...
      .frame(
        width: (dynamicTypeSize.isAccessibilitySize ? 60 : MapCardLayout.compactHeight),
        height: (dynamicTypeSize.isAccessibilitySize ? 60 : MapCardLayout.compactHeight)
      )
      .contentShape(Circle())
      .buttonStyle(.plain)
      .disabled(!isEnabled))
...
private struct PlacesMapCanvas: View {
  ...
  @State private var labelPlacements: [City.ID: PlacesMapLabelPlacement] = [:]
  ...
  private var mapMarkers: [PlacesMapMarkerPresentation] {
    ...
    return presentations.compactMap { presentation in
      ...
      guard
        presentation.recommendation != nil
          || presentation.id == selectedPlaceID
      else {
        return nil
      }
      return PlacesMapMarkerPresentation(
        ...
        color: presentation.recommendation.map {
          ({ (recommendation: PlaceRecommendation) -> Color in
  ...
  private var selectedPreviewCity: City? {
    guard let selectedPreviewID,
      previewCity?.id == selectedPreviewID
    else {
      return nil
    }
    return previewCity
  ...
  private var sunCandidatePresentations: [MapSunCandidatePresentation] {
    ...
    let transientResults =
      isPresentingSunSearch
      ? sunSearchResults
      : sunSearchResults.filter {
        savedPlaceIDsByTransientResultID[$0.id] == nil
          || $0.id == selectedSunID
      }
    for result in transientResults {
      resultsByID[result.id] = result
    }
    let candidates =
      sunCandidateCities.isEmpty
      ? transientResults.map(\.recommendation.cityWeather.city)
      : sunCandidateCities
  ...
  private var labelLayoutInputs: [PlacesMapLabelLayoutInput] {
    let savedInputs = mapMarkers.enumerated().map { index, marker in
      PlacesMapLabelLayoutInput(
        ...
        priority: ({ (id: City.ID) -> Bool in

          selectedPresentation?.id == id
        })(marker.id)
          ? 10_000
          : 1_000 - index,
        isSelected: ({ (id: City.ID) -> Bool in
    ...
    let foundInputs = sunCandidatePresentations.enumerated().compactMap {
      ...
      guard candidate.result != nil,
        ({ (candidate: MapSunCandidatePresentation) in
    ...
    let previewInputs =
      (previewCity != nil
        ...
        ) ?? (selectedPreviewCity == nil ? nil : theme.colors.secondaryText)) != nil)
    ...
    let locationInput =
      (locationCoordinate != nil
        ...
        ) ?? (isLocationSelected ? theme.colors.secondaryText : nil)) != nil)
      ? locationCoordinate.map { coordinate in
        [
          PlacesMapLabelLayoutInput(
            ...
            priority: (isLocationSelected
              && selectedPresentation == nil
              && selectedSunResult == nil
              && selectedPreviewCity == nil) ? 10_000 : 2_000,
    ...
    let tappedRegionInput =
      tappedRegionCoordinate != nil
        ...
        ) ?? (tappedRegionContext == nil ? nil : theme.colors.secondaryText)) != nil
        && (tappedRegionContext.map { !isSavedPlace($0.city) } ?? true)
  ...
  private var currentLocationCardCity: City? {
    ...
    if let recommendation = locationRecommendation,
      ({ (city: City, coordinate: CLLocationCoordinate2D) -> Bool in
        ...
        abs(city.latitude - coordinate.latitude) < 0.0001
          && abs(city.longitude - coordinate.longitude) < 0.0001
      })(recommendation.cityWeather.city, coordinate)
    {
  ...
  private var activeFloatingCard: some View {
    if let selectedPresentation {
      mapPlaceContextCard(
        ...
        displayName: selectedPresentation.place.customName
          ?? {
            let city = selectedPresentation.place.city
            let trimmedTitle =
      ...
      mapPlaceContextCard(
        city: selectedSunResult.recommendation.cityWeather.city,
        displayName: {
          let city = selectedSunResult.recommendation.cityWeather.city
          let trimmedTitle =
            city.titleName?.trimmingCharacters(
              in: .whitespacesAndNewlines
            ) ?? ""
        ...
        removeSavedPlace: {
          removeSavedPlace(selectedSunResult.recommendation.cityWeather.city)
        },
        isSaved: savedPlaceIDsByTransientResultID[selectedSunResult.id] != nil,
      )
      ...
      mapPlaceContextCard(
        ...
        displayName: {
          let trimmedTitle =
            selectedPreviewCity.titleName?.trimmingCharacters(
              in: .whitespacesAndNewlines
            ) ?? ""
          return trimmedTitle.isEmpty
            ? selectedPreviewCity.localizedDisplayName(locale: locale)
            : trimmedTitle
        }(),
      ...
      let currentLocationCardCity
    {
      ...
      mapPlaceContextCard(
        ...
        displayName: usesHomeLocation
          ? locationLabel : (locationName.isEmpty ? locationLabel : locationName),
        weather: MapPlaceWeatherPresentation(
          recommendation: locationRecommendation,
  ...
  private func mapPlaceContextCard(
    ...
    let regions =
      ({
        ...
        return (
          country,
          CountryCityCatalog.continentCountryCodes.first {
            $0.value.contains(country.iso2)
          }?.key
        )
      })(city)
  ...
  private var activeBottomSurface: some View {
    ZStack(alignment: .bottomTrailing) {
        ...
        if #available(iOS 26.0, *) {
          GlassEffectContainer(spacing: MapCardLayout.surfaceSpacing) {
            if hasFloatingCard {
              MapCard(
                size: .large(
                  horizontalPadding: dynamicTypeSize.isAccessibilitySize ? 12 : 18
                ),
                maximumWidth: UIDevice.current.userInterfaceIdiom == .pad ? 390 : 580,
              ...
              MapCard(
                size: .small,
                maximumWidth: UIDevice.current.userInterfaceIdiom == .pad ? 390 : 580,
                bottomPadding: UIDevice.current.userInterfaceIdiom == .pad
                  ? 8 : MapCardLayout.bottomPadding,
                fallbackGeometryID: Self.bottomSurfaceFallbackGeometryID,
                glassNamespace: bottomSurfaceNamespace
              ) {
      ...
      .frame(
        maxWidth: (UIDevice.current.userInterfaceIdiom == .pad ? .infinity : nil),
        alignment: .bottom
      )
  ...
  private var bottomSurfacePresentationID: String {
    ...
    if !isFindingSun && sunQuerySummary != nil && !hasFloatingCard {
    ...
    if presentations.contains(where: \.isLoading)
      && (presentations.filter { $0.recommendation != nil }).isEmpty
    {
  ...
  private var compactBottomSurface: some View {
    ...
    } else if !isFindingSun && sunQuerySummary != nil && !hasFloatingCard {
      sunResultsSummary
    } else {
      MapCapsule {
        FindSunButton(
  ...
  var body: some View {
    ...
    .onChange(
      of: (visiblePresentations
        .map(\.id)
        .sorted { $0.uuidString < $1.uuidString }), initial: true
    ) { _, _ in
    ...
    .onChange(of: mapHandoffToken, initial: true) { _, token in
      ...
      if selectsDefaultLocationOnMap {
        ...
        selectSearchPreview(previewCity.id)
        withAnimation(.smooth(duration: 0.35)) {
          position = .region(
            ({ (city: City) in
  ...
  private func mapAnnotationTapTarget(
    ...
    if (weatherMarkerColor(
      recommendation: locationRecommendation,
      isLoading: isLocationWeatherLoading || isRequestingLocationWeather
    ) ?? (isLocationSelected ? theme.colors.secondaryText : nil)) != nil, let locationCoordinate {
      targets.append(
        (
          locationCoordinate,
    ...
    return targets.first { candidate in
      ...
      return CGRect(
        x: label.point.x - label.size.width / 2,
        y: label.point.y
          + (placement == .above ? -18 : placement == .below ? 18 : 0)
          - label.size.height / 2,
    ...
    Task {
        ...
        || metadata.timeZone == nil
      {
        DeveloperDiagnostics.show(
          ...
          message:
            "Attempt 1 for \(mapTapCoordinateDescription(coordinate)) returned \("locality=" + (metadata.locality ?? "nil") + ", country=" + (metadata.countryName ?? "nil") + ", ISO=" + (metadata.isoCountryCode ?? "nil") + ", timeZone=" + (metadata.timeZone?.identifier ?? "nil")). Retrying once."
        )
      ...
      guard missingFields.isEmpty,
        ...
        let fieldList =
          formattedFields?.isEmpty == false
          ? formattedFields!
          : localizedString("place", locale: locale)
        DeveloperDiagnostics.show(
          title: "Map Tap Missing Data",
          message:
            "Attempt 2 for \(mapTapCoordinateDescription(coordinate)) could not create a queryable place. Missing: \(missingFields.joined(separator: ", ")). Metadata: \("locality=" + (metadata.locality ?? "nil") + ", country=" + (metadata.countryName ?? "nil") + ", ISO=" + (metadata.isoCountryCode ?? "nil") + ", timeZone=" + (metadata.timeZone?.identifier ?? "nil")). Catalog match: \(mappedCountry?.id ?? "none")."
        )
      ...
      let countryName =
        cleanMapTapValue(metadata.countryName)
      ...
      tappedRegionContext = MapTapRegionContext(
        ...
        locality: locality,
        country: country,
        continent: CountryCityCatalog.continentCountryCodes.first {
          $0.value.contains(country.iso2)
        }?.key
      )
      ...
      let request = MKReverseGeocodingRequest(location: location)
    {
      ...
      do {
        ...
        if let item = items.first {
          ...
          metadata = MapTapPlaceMetadata(
            locality: metadata.locality ?? other.locality,
            countryName: metadata.countryName ?? other.countryName,
            isoCountryCode: metadata.isoCountryCode
              ?? other.isoCountryCode,
            timeZone: metadata.timeZone ?? other.timeZone
          )
        } else {
          DeveloperDiagnostics.show(
            title: "Map Tap Reverse Geocoding Empty",
            message:
              "MapKit returned no map items for \(mapTapCoordinateDescription(location.coordinate))."
          )
        }
        ...
        DeveloperDiagnostics.show(
          ...
          message:
            "MapKit failed for \(mapTapCoordinateDescription(location.coordinate)): \(error.localizedDescription)"
        )
      }
    } else {
      DeveloperDiagnostics.show(
        ...
        message:
          "MapKit reverse geocoding is unavailable for \(mapTapCoordinateDescription(location.coordinate)); using CLGeocoder."
      )
    }
      ...
      do {
          ...
          .first
        {
          ...
          DeveloperDiagnostics.show(
            title: "Map Tap Reverse Geocoding Empty",
            message:
              "CLGeocoder returned no placemarks for \(mapTapCoordinateDescription(location.coordinate))."
          )
        }
      } catch is CancellationError {
        return metadata
        ...
        DeveloperDiagnostics.show(
          title: "Map Tap Reverse Geocoding Failed",
          message:
            "CLGeocoder failed for \(mapTapCoordinateDescription(location.coordinate)): \(error.localizedDescription)"
        )
      }
    }
  ...
  private func selectSunResult(_ id: City.ID) {
    ...
    pendingSavedPlaceFocusID = nil
    clearTappedRegionContext()
    if let city = sunSearchResults.first(where: { $0.id == id })?
  ...
  private func selectCurrentLocation() {
    let shouldLoadWeather =
      needsLocationWeather
  ...
  private func updateLabelPlacements(
    ...
    let projectedLabels: [PlacesMapProjectedLabel] =
      labelLayoutInputs
      .compactMap {
    ...
    while !pendingLabels.isEmpty {
      let candidatesByIndex = Dictionary(
        uniqueKeysWithValues: pendingLabels.indices.map { index in
          (
            index,
            (PlacesMapLabelPlacement.preferredOrder.compactMap {
              placement -> PlacesMapLabelCandidate? in
              let pendingLabel = pendingLabels[index]
              let rect = CGRect(
                x: pendingLabel.point.x - pendingLabel.size.width / 2,
                y: pendingLabel.point.y
                  + (placement == .above ? -18 : placement == .below ? 18 : 0)
                  - pendingLabel.size.height / 2,
                width: pendingLabel.size.width,
                height: pendingLabel.size.height
              )
              guard
                canPlaceLabel(
                  for: rect,
                  labelID: (pendingLabels[index]).input.id,
      ...
      guard
        let nextIndex = pendingLabels.indices.min(by: { lhs, rhs in
      ...
      guard !candidates.isEmpty else {
          ...
          viewportBounds.contains(projectedLabel.point)
        {
          ...
          let fallbackRect = CGRect(
            x: projectedLabel.point.x - projectedLabel.size.width / 2,
            y: projectedLabel.point.y + 18 - projectedLabel.size.height / 2,
            width: projectedLabel.size.width,
            height: projectedLabel.size.height
          ).insetBy(dx: -1, dy: -1)
  ...
  private func initializeCamera() {
      ...
      })
    {
      position = .region(
        MKCoordinateRegion(
          center: CLLocationCoordinate2D(
            latitude: selected.place.city.latitude,
            longitude: selected.place.city.longitude
          ),
          span: MKCoordinateSpan(
            latitudeDelta: 0.35,
            longitudeDelta: 0.35
      ...
      position = .region(
        PlacesMapRegionFitting.region(
          for: visiblePresentations.map { presentation in
            CLLocationCoordinate2D(
              latitude: presentation.place.city.latitude,
              longitude: presentation.place.city.longitude
            )
  ...
  private func fitSunSearchRequest(
    ...
    if request.kind == .nearMe || request.kind == .nearPlace,
      let originLatitude = request.originLatitude,
      let originLongitude = request.originLongitude
    {
      let origin = CLLocationCoordinate2D(
        latitude: originLatitude,
        longitude: originLongitude
      )
      if !coordinates.contains(where: {
    ...
    let resultSummaryHeight: CGFloat =
      dynamicTypeSize.isAccessibilitySize
      ? 60
      : MapCardLayout.compactHeight
    let resultSummaryClearance: CGFloat =
...
private struct PlacesMapMarkerLabel: View {
  ...
  var body: some View {
    Text(name)
      ...
      .offset(y: placement == .above ? -18 : placement == .below ? 18 : 0)
      .opacity(placement != .hidden ? 1 : 0)
      .allowsHitTesting(false)
  }
}
...
private struct PlacesWeatherMapDot: View {
  ...
  var body: some View {
    ...
    .scaleEffect((UIDevice.current.userInterfaceIdiom == .pad ? 1.25 : 1))
...
private enum PlacesMapRegionFitting {
  static func region(
    ...
    return
      ({
        (
          minimumLatitude: CLLocationDegrees, maximumLatitude: CLLocationDegrees,
          centerLongitude: CLLocationDegrees, longitudeSpan: CLLocationDegrees
        ) in
      ...
      })(minimumLatitude, maximumLatitude, longitudeArc.center, longitudeArc.span)
  }
  ...
  static func region(
    ...
    return MKCoordinateRegion(
      center: CLLocationCoordinate2D(
        ...
        longitude: ({ (longitude: CLLocationDegrees) -> CLLocationDegrees in
