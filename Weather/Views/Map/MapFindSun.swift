...
extension MapView {
  ...
  func beginSunSearch(
    ...
    case .country(let country):
      ...
      if model.recentSearches.countryISO2Codes.first != code {
        model.recentSearches.countryISO2Codes.removeAll { $0 == code }
        model.recentSearches.countryISO2Codes.insert(code, at: 0)
        model.recentSearches.countryISO2Codes = Array(
          model.recentSearches.countryISO2Codes.prefix(
    ...
    case .continent(let continent):
      if model.recentSearches.continents.first != continent {
        model.recentSearches.continents.removeAll { $0 == continent }
        model.recentSearches.continents.insert(continent, at: 0)
        model.recentSearches.continents = Array(
          model.recentSearches.continents.prefix(
            RecentSearchStore.maximumSuggestionCount
          )
        )
        model.recentSearches.persist()
  ...
  private func runSunSearch(
    ...
    let candidateScopeGeneration = model.mapCandidateScopeGeneration
    ...
    let hasStableCandidateContext =
      preservingCandidateContext
    ...
    Task {
      ...
      do {
        let candidates: [City]
        do {
          candidates = try await sunSearchCandidates(for: scope)
        ...
        if candidateIDs != model.activeMapCandidateIDs {
          model.activeMapCandidateIDs = candidateIDs
          model.retainWeatherScope()
        }
        ...
        weatherStore.startAttributionLoadIfNeeded()
        guard !Task.isCancelled,
          generation == sunSearchID,
          model.isCurrentMapCandidateScope(
            candidateScopeGeneration
        ...
        await withTaskGroup(of: City.ID.self) { group in
          ...
          for await cityID in group {
            ...
            _ = cityID
            rebuildSunSearchResults(
              for: model.forecastCalendar.startOfDay(for: selectedDate)
            )
          }
        }
        guard !Task.isCancelled,
  ...
  func rerankSunSearchForSelectedDate() {
    ...
    let generation = sunSearchID
    let scopeGeneration = model.mapCandidateScopeGeneration
  ...
  private func rebuildSunSearchResults(
    ...
    let recommendations = sunCandidateCities.compactMap {
      ({ (city: City, requestedDate: Date) -> PlaceRecommendation? in
    ...
    let results =
      ({ (recommendations: [PlaceRecommendation]) -> [MapSunSearchResult] in
  ...
  private func reportFindSunWeatherFailureIfNeeded(
    ...
    await missingDataAlerts.retryThenReport(
      ...
      retry: {
        ...
        await weatherStore.load(cities: candidates, forceRefresh: true)
        guard generation == sunSearchID,
          model.isCurrentMapCandidateScope(scopeGeneration)
        else {
          return
  ...
  private func sunSearchCandidates(
    ...
    case .continent(let continent):
      ...
      cities = CountryCityCatalog.spatiallyDistinctCities(
        Array(
          CountryCityCatalog.cityEntries(for: continent)
            .prefix(
              max(
                FindSunCitySamplingPolicy.resultLimit,
                FindSunCitySamplingPolicy.sourceCandidateLimit
  ...
  func saveSunResult(_ result: MapSunSearchResult) -> Bool {
    do {
      let savedID = try placesStore.savePlace(
        result.recommendation.cityWeather.city
      )
