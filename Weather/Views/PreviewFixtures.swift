...
//
  ...
  enum WeatherPreviewFixtures {
    ...
    static func model() -> WeatherModel {
      ...
      let londonWeather = weather(for: london)
      weatherStore.weatherByID[londonWeather.id] = londonWeather
      weatherStore.refreshDatesByPlaceID[londonWeather.id] = .now
    ...
    private static func weather(for city: City) -> CityWeather {
      ...
      let forecasts = (0..<10).compactMap { dayOffset -> DailyForecast? in
        guard
          ...
          let sunset = calendar.date(byAdding: .hour, value: 20, to: day)
        else {
          return nil
        }
  ...
  struct DetailViewRoutePreview: View {
    @State private var selectedDate: Date = {
      var calendar = Calendar(identifier: .gregorian)
      calendar.timeZone = WeatherPreviewFixtures.timeZone
