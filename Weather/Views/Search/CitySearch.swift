...
enum CitySearchResolutionError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .providerFailed(let provider, let place, let reason):
      switch provider {
      case .appleMaps:
        "Apple Maps could not resolve \(place): \(reason)"
      case .openMeteo:
        "Open-Meteo could not resolve \(place): \(reason)"
      }
    }
  }
}
...
private struct CitySearchPlaceCandidate {
  let cityName: String?
  let country: String?
  let countryISO2Code: String?
  let coordinate: CLLocationCoordinate2D?
...
final class CitySearchManager: NSObject, MKLocalSearchCompleterDelegate {
  ...
  private func resolvePlaceOnce(
    ...
    do {
      ...
      let candidate = CitySearchPlaceCandidate(
        ...
        countryISO2Code: ({ (code: String?) -> String? in
    ...
    guard
      candidate.cityName == nil
        ...
        || ({ (code: String?) -> String? in
          ...
          return code
        })(candidate.countryISO2Code) == nil
        || ({ (identifier: String?) -> String? in
          ...
          return identifier
        })(candidate.timeZoneIdentifier) == nil
    else {
      return try validatedPlace(candidate, resultTitle: resultTitle)
    }
  ...
  private func validatedPlace(
    ...
    guard
      let suppliedTimeZone = cleanProviderValue(
        candidate.timeZoneIdentifier
      )
    else {
