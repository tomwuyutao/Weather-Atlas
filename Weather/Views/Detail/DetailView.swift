...
struct DetailReportContent<HeaderTitle: View, SupplementaryContent: View>: View {
  ...
  var body: some View {
    GeometryReader { geometry in
      let usesLandscapeIPadSplit =
        UIDevice.current.userInterfaceIdiom
        == .pad
        && geometry.size.width > geometry.size.height
        && geometry.size.width >= LandscapeReportLayout.minimumWidth
      let topPadding =
...
private struct PlaceDetailNames {
  ...
  init(city: City?, savedPlace: SavedPlace?, locale: Locale) {
    displayName =
      savedPlace?.localizedDisplayName(locale: locale)
...
struct CurrentLocationReportContent: View {
  ...
  var body: some View {
    ...
    DetailReportContent(
      ...
      NearbySunnyPlacesSection(
        ...
        viewOnMap: {
          router.showMap(
            findingSunIn: ({ () -> MapSunQueryScope in
  ...
  private var locationForecastDates: [Date] {
    ...
    let dates = Set(
      locationWeather.dailyForecasts.compactMap { forecast in
  ...
  private func viewOnMap() {
    ...
    router.selectsDefaultLocationOnMap = false
    router.pendingMapSunHandoff = nil
    router.mapHandoffToken &+= 1
    router.selectsDefaultLocationOnMap = true
  ...
  private var errorMessage: String? {
    ...
    case .place:
      ({ () -> Bool in
        ...
        return model.placeNearbySearchOriginID == city.id
      }()) ? model.placeNearbySearchError : nil
    }
  }
}
...
struct DetailView: View {
  ...
  var body: some View {
    ...
    let forecastUnavailableMessage: String? = {
      ...
      guard let city,
        let issue = model.weatherStore.failuresByID[city.id]?.issue
      else {
        return nil
      }
...
struct LocationReportHeader<HeaderTitle: View>: View {
  ...
  private func sunStatusText(
    ...
    case .sunnyForHours(let count):
      let roundedCount = Int64(count.rounded())
      var hoursResource: LocalizedStringResource = "\(roundedCount) hours"
      hoursResource.locale = locale
      let localizedHours = String(localized: hoursResource)
        ...
        if let sunStatus {
          ...
          case .sunOutNow:
            HStack(spacing: 3) {
              ...
              Text((localizedString("Sun out now", locale: locale)))
                .font(.body.weight(.semibold))
                .foregroundStyle(theme.colors.dotSun)
            }
