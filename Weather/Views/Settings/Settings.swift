...
struct SettingsView: View {
  ...
  var body: some View {
    ...
    .preferredColorScheme(
      {
        ...
        case .automatic, .automaticBlack: nil
        }
      }()
    )
    ...
    .alert(
      ...
      Text(
        "Translate the names of your saved places to \(languageDisplayName(for: pendingLanguageCode ?? appLanguage))? Names without a translation stay unchanged."
      )
    }
  }
    ...
    Form {
      Section {
        navigationRow(
          "Units",
          value: {
            switch TemperatureUnit(rawValue: temperatureUnit) ?? .systemDefault {
            case .celsius:
              localizedString("Celsius (°C)", locale: locale)
            case .fahrenheit:
              localizedString("Fahrenheit (°F)", locale: locale)
            }
          }(),
          systemImage: "ruler",
          destination: .units
      ...
      Section {
        ForEach(
          ...
          selectionRow(
            ...
            isSelected: (TemperatureUnit(rawValue: temperatureUnit) ?? .systemDefault) == unit
          ) {
          ...
          selectionRow(
            {
              switch unit {
              case .kilometers:
                localizedString("Kilometers (km)", locale: locale)
              case .miles:
                localizedString("Miles (mi)", locale: locale)
              }
            }(),
            isSelected: (DistanceUnit(rawValue: distanceUnit)
              ?? .kilometers) == unit
          ) {
  ...
  private var textSizeMenuRow: some View {
    Menu {
      Button {
        ...
        HStack {
          Text(localizedString("Follow System", locale: locale))
          Spacer(minLength: 8)
  ...
  private func translateSavedPlaceNames(to targetLanguageCode: String) async {
    ...
    for place in model.placesStore.allPlaces {
      guard
        let translatedName = await CityNameLocalizationCatalog.localizedName(
          matchingSavedPlaceLabel: (place.customName
            ?? place.city.name.trimmingCharacters(in: .whitespacesAndNewlines))
