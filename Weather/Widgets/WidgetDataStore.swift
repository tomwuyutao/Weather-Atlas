...
enum WidgetDataStore {
  ...
  static func localizedText(for key: String) -> String {
    ...
    static func save(_ catalog: WidgetDataCatalog) {
      ...
      let defaults = UserDefaults.standard
      publishedCatalog.followsSystemTextSize =
        defaults.object(
          forKey: AppTextSizePolicy.useSystemKey
        ) == nil
        ? true
    ...
    private static func defaultLocationIdentityMatches(
      ...
      guard
        previous.resolvedDefaultLocationKind
          == current.resolvedDefaultLocationKind,
        previous.currentLocation?.timeZoneIdentifier
          == current.currentLocation?.timeZoneIdentifier
      else {
        return false
      }
      ...
      case (nil, nil):
        return true
      case (let previousCity?, let currentCity?):
    ...
    private static func retiredWidgetCities(
      ...
      let previousCities =
        (previousCatalog?.cities ?? [])
        + (previousCatalog?.retiredCities ?? [])
      ...
      return previousCities.compactMap { city in
        guard city.id != currentLocationIdentifier,
          ...
          seenIdentifiers.insert(city.id).inserted
        else {
          return nil
        }
    ...
    static func removeAll() {
      ...
      defaults.set(
        UUID().uuidString,
        forKey: "weatherAtlas.widgetResetEpoch"
      )
      WidgetCenter.shared.reloadAllTimelines()
    }
  ...
  extension WidgetDataCity {
    ...
    var widgetSunnyWindowIssue: WeatherDataIssue? {
      ...
      guard !(Array((sunnyWindowDays ?? []).prefix(10))).isEmpty else {
    ...
    func applying(
      ...
      if snapshot.representsLocalDay(containing: referenceDate) {
        ...
        guard
          let day = snapshot.sunnyWindowDays?.first(where: {
          ...
          dayWeatherConditions.allSatisfy({ $0.weather != nil })
        else {
          return nil
        }
