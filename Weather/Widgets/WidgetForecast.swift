...
struct SunnyHoursLockScreenProvider: AppIntentTimelineProvider {
  ...
  private func refreshedCity(
    ...
    guard
      let selectedCatalogCity = selectedCity(
        for: configuration,
    ...
    let capturedLanguageIdentifier: String = {
      guard let identifier = capturedCatalog?.appLanguageIdentifier,
        !identifier.isEmpty
      else {
    ...
    let capturedDefaultLocationKind =
      selectedCatalogCity.id
        == WidgetDataStore.currentLocationIdentifier
      ? capturedCatalog?.resolvedDefaultLocationKind ?? .currentLocation
      : nil
    ...
    let selectedEntity =
      configuration.city
      ?? .defaultLocation(in: capturedCatalog)
    let resolvesDeviceLocation =
      selectedEntity.id == WidgetDataStore.currentLocationIdentifier
      && (capturedCatalog?.resolvedDefaultLocationKind ?? .currentLocation)
        == .currentLocation
    guard
...
extension SunnyHoursLockScreenProvider {
  ...
  func selectedCity(
    ...
    WidgetForecastStore.prune(
      keeping: Set(
        [WidgetDataStore.currentLocationIdentifier]
          + catalog.cities.flatMap { city in
            var seen: Set<String> = []
            return ([city.id] + (city.legacyIdentifiers ?? []))
              .filter {
    ...
    }), savedCity.widgetIdentityIssue == nil {
      return savedCity
    }
    if let retiredCity = (catalog.retiredCities ?? []).first(where: {
      $0.id == selectedEntity.id
    }) {
  ...
  func selectionStillMatches(
    ...
    let currentDefaultLocationKind =
      currentCity.id
        == WidgetDataStore.currentLocationIdentifier
    ...
    if resolvesDeviceLocation {
      return currentCity.id == WidgetDataStore.currentLocationIdentifier
        && (configuration.city ?? .defaultLocation(in: catalog)).id
          == WidgetDataStore.currentLocationIdentifier
        && (catalog?.resolvedDefaultLocationKind ?? .currentLocation)
          == .currentLocation
        ...
        matching: {
          guard
            ...
            let candidate = city.applying(
              ...
              preservesResolvedCityName: preservesResolvedCityName,
              at: referenceDate
            ), candidate.widgetCurrentIssue == nil
          else {
            return false
          }
...
enum WidgetTimelinePlanner {
  ...
  static func timeline(
    ...
    let timelineHours =
      (snapshot.hourlyWeatherConditions ?? [])
  ...
  var hasDirectWeatherPresentation: Bool {
    ...
    return hourlyConditions.allSatisfy { $0.weather != nil }
      ...
      && sunnyWindowDays.allSatisfy { day in
        ...
        let completeHoursAreValid =
          day.hourlyWeatherConditions?
...
enum WidgetForecastStore {
  ...
  private static func retainedSnapshot(
    ...
    guard snapshot.resetEpoch == WidgetResetEpoch.current,
      ...
      snapshot.representedLocalDate != nil,
      age >= 0,
      age < WidgetForecastCachePolicy.retentionInterval
    else {
      UserDefaults.standard.removeObject(forKey: key)
      return nil
    }
...
actor WidgetWeatherOperationCoordinator {
  ...
  nonisolated private static func performRequest(
    ...
    case .forecastFallback:
      ...
      guard
        let nearestHour = hourly.forecast.min(by: {
...
enum WidgetWeatherFetcher {
  ...
  static func response(
    ...
    guard
      let forecastEndDate = calendar.date(
        byAdding: .day,
        value: 10,
    ...
    do {
      let timeout = try
        ({ (preferred: Duration, deadline: ContinuousClock.Instant) throws -> Duration in
      ...
      if Self.isTerminalWeatherRequestError(error)
        || ({
          ...
          if case .timedOut = fetchError { return true }
          return false
        })()
      {
        throw error
