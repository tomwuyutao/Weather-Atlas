...
//  Weather
//
//  Purpose: Draws the hourly cloud-cover timeline used in city detail cards.
//
...
private struct TimelineChartBody: View {
    ...
    private func pointValueLabelColor(for point: ChartPoint) -> Color {
        point.isPast ? .secondary.opacity(0.38) : AppTheme.shared.colors.primaryText
    }
    ...
    func chartBackgroundIfNeeded(_ enabled: Bool) -> some View {
        if enabled {
            self
                ...
                .background(
                    AppTheme.shared.colors.chartPanelFill,
