...
final class MissingDataAlertCenter {
  ...
  var currentAlert: MissingDataAlert?
  ...
  func retryThenReport(
    ...
    let trimmedRecoveryKey = recoveryKey.trimmingCharacters(
      in: .whitespacesAndNewlines
    )
    let normalizedRecoveryKey =
      trimmedRecoveryKey.isEmpty
      ? report.key
      : trimmedRecoveryKey
  ...
  private func detachReportKeyFromRecovery(_ reportKey: String) {
    guard let recoveryKey = recoveryKeyByReportKey.removeValue(forKey: reportKey),
      var recovery = recoveries[recoveryKey]
    else {
      return
    }
