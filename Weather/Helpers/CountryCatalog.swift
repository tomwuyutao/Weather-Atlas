...
struct CountryPlacesOption: Identifiable, Hashable {
  ...
  func matchesSearchQuery(_ query: String, locale: Locale) -> Bool {
    ...
    let localizedMatch =
      localizedName(locale: locale).range(
        of: trimmedQuery,
        options: options,
        locale: locale
      ) != nil
...
struct CountryCityCatalogEntry: Identifiable, Hashable {
  ...
  func representsSamePlace(as other: CountryCityCatalogEntry) -> Bool {
    let normalizedCityName = city.trimmingCharacters(
      in: .whitespacesAndNewlines
    ).folding(
      options: [.caseInsensitive, .diacriticInsensitive, .widthInsensitive],
      locale: Locale(identifier: "en_US_POSIX")
    )
    let otherNormalizedCityName = other.city.trimmingCharacters(
      in: .whitespacesAndNewlines
    ).folding(
      options: [.caseInsensitive, .diacriticInsensitive, .widthInsensitive],
      locale: Locale(identifier: "en_US_POSIX")
    ...
    guard iso2 == other.iso2,
      ...
      normalizedCityName == otherNormalizedCityName
    else {
      return false
    }
...
enum CountryCityCatalog {
  ...
  static func countries(
    ...
    let allCountries = catalog.countriesByCode.values.sorted {
  ...
  /// count.
  static func spatiallyDistinctCities(
  ...
  static let continentCountryCodes: [ContinentPlacesOption: Set<String>] = [
    .europe: [
  ...
  static let catalog: CatalogData = {
    guard
      let url = Bundle.main.url(forResource: "country_city_coordinates", withExtension: "csv")
        ?? Bundle.main.url(
      ...
      DeveloperDiagnostics.show(
        ...
        message:
          "The bundled country_city_coordinates.csv file is missing. Country and continent searches are unavailable."
      )
      return CatalogData(
        countriesByCode: [:],
    ...
    do {
      ...
      DeveloperDiagnostics.show(
        ...
        message:
          "The bundled country_city_coordinates.csv file could not be read: \(error.localizedDescription)"
      )
      return CatalogData(
        countriesByCode: [:],
    ...
    for (rowIndex, line) in csv.split(whereSeparator: \.isNewline).dropFirst().enumerated() {
      ...
      guard fields.count == 7,
        let city = { () -> String? in
          let value = fields[0].trimmingCharacters(
            in: .whitespacesAndNewlines
          )
        ...
        let longitude = { () -> Double? in
          guard let value = Double(fields[4]),
            value.isFinite,
        ...
        DeveloperDiagnostics.show(
          ...
          message:
            "The bundled country_city_coordinates.csv row \(rowIndex + 2) is malformed and cannot be loaded."
        )
        continue
      }
    ...
    let countriesByCode: [String: CountryPlacesOption] = grouped.reduce(
      ...
      for city in rankedCities
      where !sortedCities.contains(
