...
final class SavedPlacesWeatherStore {
  ...
  @ObservationIgnored
  private var inFlightByID: [City.ID: (token: UUID, task: Task<CityWeather?, Never>)] = [:]
  ...
  @ObservationIgnored var refreshDatesByPlaceID: [City.ID: Date] = [:]
  ...
  func weather(for placeID: City.ID) -> CityWeather? {
    ...
    guard let refreshDate = refreshDatesByPlaceID[placeID],
      {
        let age = Date.now.timeIntervalSince(refreshDate)
        return age >= 0
          && age < PlaceWeatherSnapshotCache.maximumRetentionInterval
      }()
    else {
      return nil
    }
  ...
  func adoptRetainedLocationWeather(
    ...
    guard sourcePlaceID != city.id,
      ...
      let sourceRefreshDate = refreshDatesByPlaceID[source.id]
    else {
      return nil
    }
    ...
    guard
      destination.distance(from: sourceLocation)
  ...
  func load(
    ...
    var seenPlaceIDs: Set<City.ID> = []
    let uniqueCities = cities.filter {
      seenPlaceIDs.insert($0.id).inserted
    }
    guard !uniqueCities.isEmpty else { return }
    #if DEBUG
      let startedElapsed = Date().timeIntervalSince(debugStartedAt)
      let startedDetail =
        "[WeatherBatch +\(String(format: "%.2f", startedElapsed))s] started for \(uniqueCities.count) cities"
      Self.weatherBatchLogger.notice("\(startedDetail, privacy: .public)")
    #endif
    ...
    #if DEBUG
      let awaitingElapsed = Date().timeIntervalSince(debugStartedAt)
      let awaitingDetail =
        "[WeatherBatch +\(String(format: "%.2f", awaitingElapsed))s] awaiting \(tasks.count) new or coalesced forecast requests"
      Self.weatherBatchLogger.notice("\(awaitingDetail, privacy: .public)")
    #endif
    ...
    for city in uniqueCities {
      ...
      while let request = inFlightByID[city.id],
      ...
      let failureSummary =
        ...
        .sorted()
        .joined(separator: ", ")
      let completedElapsed = Date().timeIntervalSince(debugStartedAt)
      let completedDetail =
        "[WeatherBatch +\(String(format: "%.2f", completedElapsed))s] completed: \(availableCount)/\(uniqueCities.count) available; failures: \(failureSummary.isEmpty ? "none" : failureSummary)"
      Self.weatherBatchLogger.notice("\(completedDetail, privacy: .public)")
    #endif
  }
  ...
  func startAttributionLoadIfNeeded() {
  ...
  private func performRequest(
    ...
    defer {
      ...
      if requestTokens[city.id] == token {
        requestTokens[city.id] = nil
    ...
    refreshDatesByPlaceID[city.id] = Date()
    scheduleCacheExpiry()
    if cache.fileURL != nil {
      cacheIsDirty = true
      if cacheBatchDepth == 0 {
  ...
  private func shouldRefresh(placeID: City.ID, now: Date = Date()) -> Bool {
    ...
    guard let weather = weatherByID[placeID],
      let refreshDate = refreshDatesByPlaceID[placeID]
    else {
      return true
    }
  ...
  private func beginRequest(for placeID: City.ID) -> UUID {
    ...
    if weatherByID.removeValue(forKey: placeID) != nil {
      weatherRevision &+= 1
    }
  ...
  private func restoreRetainedWeatherAfterFailure(
    ...
    guard let refreshDate = refreshDatesByPlaceID[placeID],
      ...
      let weather = cachedWeather.toCityWeather()
    else {
  ...
  /// forecast. In-flight replacement requests remove only the visible value.
  private func discardCachedWeather(for placeID: City.ID) {
    let removedWeather = weatherByID.removeValue(forKey: placeID) != nil
    let removedCachedWeather =
      cachedWeatherByID.removeValue(
        forKey: placeID
      ) != nil
    let removedRefreshDate =
      refreshDatesByPlaceID.removeValue(
        forKey: placeID
      ) != nil
  ...
  func scheduleCacheExpiry(now: Date = Date()) {
    cacheExpiryTask?.cancel()
    guard
      let nextExpiry = refreshDatesByPlaceID.values
        .map({
  ...
  private func beginCacheBatch() {
    if cacheBatchDepth == 0, cache.fileURL != nil {
      cacheOperation &+= 1
      let operation = cacheOperation
      let writer = cacheWriter
      Task {
...
nonisolated struct PlaceWeatherSnapshotCache: Sendable {
  ...
  func load(now: Date = .now) -> PlaceWeatherSnapshot? {
    ...
    let expiredOrUndatedIDs = Set(
      placeOrder.filter { placeID in
        guard let refreshDate = refreshDatesByPlaceID[placeID] else {
          return true
        }
        let age = now.timeIntervalSince(refreshDate)
        return age < 0 || age >= Self.maximumRetentionInterval
      })
