...
actor CitiesCatalog {
      ...
      let match =
        ...
        .max { lhs, rhs in
          if lhs.population != rhs.population {
            switch (rhs.population, lhs.population) {
  ...
  func mostPopulousCities(
    ...
    let cities = try await catalogData()
    ...
    for (index, city) in cities.enumerated() {
      ...
      guard distance <= radiusKilometers,
        distance > minimumDistanceKilometers
      else {
        continue
      }
      candidates.append(
    ...
    return Array(
      candidates.sorted {
        if $0.city.population != $1.city.population {
          switch ($0.city.population, $1.city.population) {
          case (let lhs?, let rhs?): return lhs > rhs
          case (_?, nil): return true
          case (nil, _?), (nil, nil): return false
          }
  ...
  func cities(
    ...
    let lowerLongitude = {
      let normalized = rawLowerLongitude.truncatingRemainder(dividingBy: 360)
      if normalized > 180 { return normalized - 360 }
      if normalized < -180 { return normalized + 360 }
      return normalized
    }()
    let upperLongitude = {
      let normalized = rawUpperLongitude.truncatingRemainder(dividingBy: 360)
      if normalized > 180 { return normalized - 360 }
      if normalized < -180 { return normalized + 360 }
      return normalized
    }()
    ...
    return Array(
      matches.sorted {
        if $0.population != $1.population {
          switch ($0.population, $1.population) {
          case (let lhs?, let rhs?): return lhs > rhs
          case (_?, nil): return true
          case (nil, _?), (nil, nil): return false
          }
  ...
  nonisolated private static func distanceKilometers(
    ...
    let latitudeDelta = (destinationLatitude - sourceLatitude) * .pi / 180
    let longitudeDelta = (destinationLongitude - sourceLongitude) * .pi / 180
    let sourceLatitudeRadians = sourceLatitude * .pi / 180
